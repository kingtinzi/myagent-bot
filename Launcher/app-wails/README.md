...
- **graph-memory 侧车**：`config.graph-memory.json` 必须与主 `config.json` 同目录（默认 **`~/Library/Application Support/OpenClaw/`**），网关才会加载；仅放在 `.app/Contents/Resources/config/` 里**不会**自动生效，除非 Launcher 已将其复制到该目录，或你设置了 **`PINCHBOT_GRAPH_MEMORY_CONFIG`** 指向某绝对路径。当前 Launcher 在启动网关前：若包内存在 **`Contents/Resources/config/config.graph-memory.json`**（或安装根 **`config/config.graph-memory.json`**）且用户主目录**尚无**同名文件，会**种子复制**一份（已存在则**不覆盖**）。
