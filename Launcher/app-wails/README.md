...
- **主程序**：用户只需运行 **launcher-chat.exe**（macOS 为 **launcher-chat.app**）；启动后会在**当前桌面主进程内**托管聊天网关（18790），并在 **与 .app 同级的** `config/platform.env` 文件存在且能找到 **platform-server** 可执行文件时，自动拉起 **platform-server**（默认 `http://127.0.0.1:18791`）。
- **平台服务未随应用启动？** 需同时满足：① 存在 **`{launcher-chat.app 所在文件夹}/config/platform.env`**（可从 `Platform/config/platform.example.env` 复制并填写）；② **`platform-server` 在 PATH 中**，或与 `Contents/MacOS/launcher-chat` 同目录，或与 `.app` 同级目录下的 `Platform/` 中（本仓库开发构建时会把仓库根下 `Platform/`、`PinchBot/build/` 也加入查找）。缺任一项则不会自动启动，登录会提示平台不可用。
...
3. **双击 .app 后闪退**  
   - 常见原因：① 包内或 **App Translocation 只读卷** 无法写入数据。当前 **`.app` 启动时默认将 PinchBot 数据放在** **`~/Library/Application Support/PinchBot/`**（始终可写）。② 仍请使用 **`wails build` / `package-macos.sh` 重新打包** 后再试。③ 可选：对 `.app` 执行 `xattr -cr launcher-chat.app`，或将整包移到「应用程序」文件夹再双击。
...
2. 依次构建 PinchBot、platform-server、Wails Launcher；
3. 在 **`dist/PinchBot-<git版本>-macOS-<amd64|arm64>/`** 生成可直接交给用户的目录：`launcher-chat.app`、`pinchbot`、`platform-server`、`config/`、`README.txt`。
