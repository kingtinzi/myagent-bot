...
2. **收件人**：
   - 用户说「发给我」「发到我的邮箱」「发给主人」时，先检查 **当前实际工作区** 的 `workspace/USER.md` 是否已经记录了该用户**明确提供过**的邮箱地址。
   - 只有当 `USER.md` 中的邮箱显然是当前用户主动提供、且不是模板占位内容时，才能直接使用。
   - 若用户明确说了「发给 xxx@yy.com」，则使用该地址。
   - 若 `USER.md` 没有可靠邮箱、内容仍是模板占位、或无法确认当前会话用户与记录匹配，必须先向用户确认收件地址，不能猜测或默认使用仓库里的模板信息。
