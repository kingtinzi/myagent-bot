...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("PinchBot")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@PinchBot hello", want: true},
		{text: "hi @PinchBot:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bPinchBot\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@PinchBot:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@PinchBot:matrix.org hello", want: "hello"},
		{in: "@PinchBot, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@PinchBot:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@PinchBot:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@PinchBot:matrix.org">PinchBot</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40PinchBot%3Amatrix.org">PinchBot</a> hello`,
			},
			want: true,
		},
	}
