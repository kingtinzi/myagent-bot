...
func TestProviderChat_ParsesToolCallsTopLevelShape(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := map[string]any{
			"choices": []map[string]any{
				{
	...
	if out.ToolCalls[0].Name != "lobster" {
		t.Fatalf("ToolCalls[0].Name = %q, want %q", out.ToolCalls[0].Name, "lobster")
	}
	if out.ToolCalls[0].Arguments["pipeline"] != "commands.list" {
		t.Fatalf("ToolCalls[0].Arguments[pipeline] = %v, want commands.list", out.ToolCalls[0].Arguments["pipeline"])
	}
}
