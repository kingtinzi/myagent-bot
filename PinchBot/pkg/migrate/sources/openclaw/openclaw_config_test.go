...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.PinchBot/workspace" {
		t.Errorf("expected workspace '~/.PinchBot/workspace', got '%s'", workspace)
	}
...
func TestConvertToPinchBot(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToPinchBot("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.PinchBot/workspace" {
		t.Errorf("expected workspace '~/.PinchBot/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToPinchBotWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToPinchBot("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToPinchBotWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToPinchBotWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	picoCfg := &pinchbotconfig{
		Agents: AgentsConfig{
			Defaults: AgentDefaults{
				Provider:  "anthropic",
				ModelName: "claude-sonnet-4-20250514",
				Workspace: "~/.PinchBot/workspace",
			},
			List: []AgentConfig{
				{
					ID:      "main",
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.PinchBot/workspace" {
		t.Errorf("expected workspace '~/.PinchBot/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
