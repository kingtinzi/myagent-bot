...
import (
	...
	"github.com/sipeed/pinchbot/pkg/commands"
...
// SetUsageLogger injects a usage logger for token/task recording (optional).
func (al *AgentLoop) SetUsageLogger(l *usage.Logger) {
	al.usageLogger = l
}
...
const nudgeUseToolsMessage = "请使用你的工具完成上述请求，不要仅回复文字。"

// userMessageSuggestsAction returns true when the message likely asks the agent to perform an action
// (e.g. send email, run something), so we may nudge once to use tools instead of just replying.
func userMessageSuggestsAction(s string) bool {
	s = strings.ToLower(strings.TrimSpace(s))
	actionPhrases := []string{
		"发邮件", "发信", "发一封", "发一份", "发送邮件", "用邮箱发",
		"send email", "send mail", "send a mail", "发到", "邮箱",
	}
...
retry:
	...
	if !opts.NoHistory {
		...
		if plugins.GraphMemoryRuntimeActive(al.cfg) {
			var compressed bool
	...
	if opts.NudgeRetryDone {
		currentUserMessage = "" // retry: history already has original + nudge, do not append again
	}
	...
	// 1b. Graph memory (Go-native): recall + assemble conversation tail
	if !opts.NoHistory && plugins.GraphMemoryRuntimeActive(al.cfg) && len(messages) > 1 {
		gctx, gcancel := context.WithTimeout(ctx, 45*time.Second)
		plugins.EmitGraphMemoryBeforeAgentStart(gctx, al.cfg, currentUserMessage, opts.SessionKey)
		add, tail, aerr := plugins.AssembleGraphMemory(gctx, al.cfg, plugins.DefaultGraphMemoryEngineID, opts.SessionKey, messages[1:], 0)
		if aerr != nil {
	...
	// 4. One gentle retry when model did not use tools on first turn (empty reply, or text-only reply to an action request)
	if iteration == 1 && !opts.NudgeRetryDone && !opts.NoHistory {
		doNudge := finalContent == "" ||
			(finalContent != "" && userMessageSuggestsAction(opts.UserMessage))
		if doNudge {
			opts.NudgeRetryDone = true
			agent.Sessions.AddMessage(opts.SessionKey, "user", nudgeUseToolsMessage)
			logger.InfoCF("agent", "First turn had no tool calls; retrying once with nudge to use tools",
				map[string]any{"empty_response": finalContent == "", "user_suggests_action": userMessageSuggestsAction(opts.UserMessage)})
			goto retry
		}
	}
	...
	// 6b. Graph memory afterTurn (ingest + async native extract/maintain)
	if !opts.NoHistory && plugins.GraphMemoryRuntimeActive(al.cfg) {
		atCtx, atCancel := context.WithTimeout(context.Background(), 60*time.Second)
		full := agent.Sessions.GetHistory(opts.SessionKey)
		plugins.AfterTurnGraphMemory(atCtx, al.cfg, plugins.DefaultGraphMemoryEngineID, opts.SessionKey, full, histAtLoopStart, false)
		atCancel()
	}
...
func (al *AgentLoop) runLLMIteration(
	...
	for iteration < agent.MaxIterations {
		...
		// Critical threshold (e.g. 80%): force checkpoint report and human-in-the-loop
		criticalThreshold := (agent.MaxIterations * CriticalThresholdPercent) / 100
		if criticalThreshold < 1 {
			criticalThreshold = 1
		}
		if iteration >= criticalThreshold {
			report := BuildReportFromMessages(messages, iteration, agent.MaxIterations)
			if err := SaveCheckpoint(agent.Workspace, opts.SessionKey, report); err != nil {
				logger.WarnCF("agent", "Failed to save checkpoint", map[string]any{"error": err.Error(), "session_key": opts.SessionKey})
			} else {
				// Trim session to summary only (context pruning)
				summaryContent := report.SummaryForContext + "\n\n" + CheckpointPrompt
				originalUser := report.OriginalUserMessage
				if originalUser == "" {
					for _, m := range messages {
						if m.Role == "user" && strings.TrimSpace(m.Content) != "" {
							originalUser = m.Content
							break
						}
					}
				}
				trimmedHistory := []providers.Message{
					{Role: "user", Content: originalUser},
					{Role: "assistant", Content: summaryContent},
				}
				agent.Sessions.SetHistory(opts.SessionKey, trimmedHistory)
				_ = agent.Sessions.Save(opts.SessionKey)
			}
			finalContent = report.FormatReport() + "\n\n" + CheckpointPrompt
		...
		// When tools were skipped due to repeated failures, add a hint so the model gives a direct reply.
		messagesForLLM := messages
		if skipToolsDueToErrors {
			messagesForLLM = make([]providers.Message, 0, len(messages)+1)
			messagesForLLM = append(messagesForLLM, messages...)
			messagesForLLM = append(messagesForLLM, providers.Message{
				Role:    "user",
				Content: "（工具执行多次失败，请直接回复用户：简要说明暂时无法完成该操作，建议稍后重试或换一种方式说明需求。）",
			})
		}
		...
		logger.DebugCF("agent", "Full LLM request",
			map[string]any{
				"iteration":     iteration,
				"messages_json": formatMessagesForLog(messagesForLLM),
		...
		if al.usageLogger != nil {
			...
			r.Prompt = utils.Truncate(formatMessagesForStorage(messages), usage.MaxStoredPromptLen)
			completion := response.Content
			if response.ReasoningContent != "" {
				completion = response.ReasoningContent + "\n\n" + completion
			}
			...
			r.Completion = utils.Truncate(completion, usage.MaxStoredPromptLen)
			al.usageLogger.Record(r)
		}
		...
		if len(response.ToolCalls) == 0 {
			...
			// Task completed normally; remove checkpoint if present so next run is fresh
...
func (al *AgentLoop) forceCompression(agent *AgentInstance, sessionKey string) {
	...
	if plugins.GraphMemoryRuntimeActive(al.cfg) {
		...
		plugins.SessionEndGraphMemory(endCtx, al.cfg, sessionKey)
		endCancel()
	}
...
// countToolErrorResultsSinceLastUser returns how many tool-result messages since the last
// user message look like errors (e.g. "failed", "timeout", "deadline exceeded"). Used to
// stop retry loops when a tool keeps failing in the current turn, without carrying over
// failures from previous turns in the same session.
func countToolErrorResultsSinceLastUser(messages []providers.Message) int {
	lastUser := -1
	for i := len(messages) - 1; i >= 0; i-- {
	...
	for _, m := range messages[start:] {
		...
		c := lower(m.Content)
		if strings.Contains(c, "failed") || strings.Contains(c, "error") ||
			strings.Contains(c, "deadline") || strings.Contains(c, "timeout") ||
			strings.Contains(c, "exceeded") {
			n++
		}
	}
	return n
...
// formatMessagesForStorage serializes messages to a single string for usage storage (full content, caller truncates).
func formatMessagesForStorage(messages []providers.Message) string {
	if len(messages) == 0 {
		return ""
	}
