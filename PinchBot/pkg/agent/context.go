...
func NewContextBuilder(workspace string) *ContextBuilder {
	...
	builtinSkillsDir := strings.TrimSpace(os.Getenv("PinchBot_BUILTIN_SKILLS"))
	if builtinSkillsDir == "" {
		wd, _ := os.Getwd()
...
func (cb *ContextBuilder) getIdentity() string {
	...
	return fmt.Sprintf(`# PinchBot 🦞

You are PinchBot, a helpful AI assistant.
...
1. **ALWAYS use tools** - When you need to perform an action (schedule reminders, send messages, execute commands, etc.), you MUST call the appropriate tool. Do NOT just say you'll do it or pretend to do it. In particular: when the user asks to send an email, you MUST call the email send tool (name in tool list: mcp_email_send_email or similar); never reply with only text describing what you would send—actually invoke that tool.
...
func (cb *ContextBuilder) BuildMessages(
	...
	// Add current user message (include when content is non-empty or when there are attachments, e.g. launcher files)
