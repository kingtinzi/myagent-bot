...
func TestToolRegistry_Clone(t *testing.T) {
	...
	// Parent mutation should not affect clone.
	r.Register(newMockTool("spawn", "spawns subagent"))
	if _, ok := clone.Get("spawn"); ok {
		t.Error("expected clone to not include tools registered after cloning")
	}

	// Clone mutation should not affect parent.
	clone.Register(newMockTool("custom", "custom tool"))
	if _, ok := r.Get("custom"); ok {
		t.Error("expected parent to not include tools registered on clone")
	}
}
