...
const (
	...
	userAgentHonest = "PinchBot AI assistant (+https://github.com/sipeed/pinchbot)"
	...
	maxRedirects     = 5
	truncationNotice = "[Content truncated due to size limit]"
)
...
func (p *SearXNGSearchProvider) Search(ctx context.Context, query string, count int) (string, error) {
	...
	// Format results in standard PinchBot format
...
// allowPrivateWebFetchHosts is test-only override for local/loopback fixtures.
...
func (t *WebFetchTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	// Retry once with an honest UA when Cloudflare explicitly serves a challenge.
	if resp.StatusCode == http.StatusForbidden && strings.EqualFold(resp.Header.Get("Cf-Mitigated"), "challenge") {
		resp2, body2, err2 := doFetch(userAgentHonest)
		...
		if err2 != nil {
			...
			return ErrorResult(fmt.Sprintf("failed to read response: %v", err2))
		}
...
// newSafeDialContext re-resolves DNS at connect time to mitigate DNS rebinding.
...
// isObviousPrivateHost is a no-DNS preflight check for local/private hosts.
...
// isPrivateOrRestrictedIP returns true for non-public address space.
