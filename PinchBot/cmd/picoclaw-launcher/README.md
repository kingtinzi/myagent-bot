...
A standalone launcher for PinchBot, providing visual JSON editing and OAuth provider authentication management.
...
# Run with default config path (~/.pinchbot/config.json)
./PinchBot-launcher
...
The **gateway** (`PinchBot gateway`) and the **launcher** (this program) are two separate processes:
...
```bash
# Windows (from repo root PinchBot/)
go build -o build/PinchBot-launcher.exe ./cmd/picoclaw-launcher/
build\PinchBot-launcher.exe
...
```
Usage: PinchBot-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.pinchbot/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.pinchbot/config.json"
}
```
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the PinchBot-config UI.
...
Gets the running status of the `PinchBot gateway` process.
...
Starts the `PinchBot gateway` process in the background.
...
Stops the running `PinchBot gateway` process.
