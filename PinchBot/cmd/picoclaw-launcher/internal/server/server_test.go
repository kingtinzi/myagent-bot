...
func TestGetConfig_MissingFile_ReturnsDefault(t *testing.T) {
	mux := http.NewServeMux()
	RegisterConfigAPI(mux, "/tmp/nonexistent-PinchBot-launcher-test/config.json")
...
func TestDefaultConfigPath(t *testing.T) {
	...
	// Should contain .pinchbot and end with config.json (exe_dir/.pinchbot or home/.pinchbot)
	if !strings.Contains(path, ".pinchbot") || !strings.HasSuffix(path, "config.json") {
		t.Errorf("expected path to contain .pinchbot and end with config.json, got %q", path)
	}
}
