...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					pinchbotconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model pinchbotconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []pinchbotconfig.ModelConfig) {
...
func isModelValid(model pinchbotconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *pinchbotconfig.ModelConfig) {
	if model == nil {
		return
	}
