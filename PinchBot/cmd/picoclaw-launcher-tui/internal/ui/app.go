...
import (
	...
	configstore "github.com/sipeed/pinchbot/cmd/picoclaw-launcher-tui/internal/config"
...
func Run() error {
	...
	if cfg == nil {
		cfg = pinchbotconfig.DefaultConfig()
	}
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open PinchBot agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("PinchBot", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("PinchBot", "gateway")
