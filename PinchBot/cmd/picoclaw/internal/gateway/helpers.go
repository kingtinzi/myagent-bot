...
func gatewayCmd(debug bool) error {
	...
	if err := internal.EnsurePinchBotHome(); err != nil {
		return fmt.Errorf("error ensuring .pinchbot home: %w", err)
	}
	cfg, err := internal.LoadConfig()
	if err != nil {
		return fmt.Errorf("error loading config: %w", err)
	}
	// Resolve relative workspace to absolute (e.g. "workspace" -> exe_dir/.pinchbot/workspace) so all pkgs see the same path.
	cfg.Agents.Defaults.Workspace = internal.ResolveWorkspacePath(cfg)
	workspacePath := cfg.WorkspacePath()
	if _, err := os.Stat(workspacePath); os.IsNotExist(err) {
		if err := onboard.CreateWorkspaceTemplates(workspacePath); err != nil {
			log.Printf("Warning: create workspace templates: %v", err)
		}
	}
	...
	usageLogger := usage.NewLogger(workspacePath)
	...
	usageHandler := usage.NewHandler(workspacePath)
