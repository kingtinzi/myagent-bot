...
import (
	...
	"github.com/sipeed/pinchbot/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to PinchBot",
		Args:  cobra.NoArgs,
		Example: `  PinchBot migrate
  PinchBot migrate --from openclaw
  PinchBot migrate --dry-run
  PinchBot migrate --refresh
  PinchBot migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.PinchBot)")
