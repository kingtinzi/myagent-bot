...
dist/
  OpenClaw-1.0.0-Windows-x86_64/
    ...
    PinchBot-launcher.exe   # 配置服务（端口 18800，由主程序自动启动）
    PinchBot.exe            # 网关（端口 18790，由主程序自动启动）
    config/
...
- 配置：`%USERPROFILE%\.pinchbot\config.json`
- 认证：`%USERPROFILE%\.pinchbot\auth.json`
- 工作区：`%USERPROFILE%\.pinchbot\workspace\`（配置里 `agents.defaults.workspace` 默认即此路径）
...
**Workspace 的两种用法**：  
1）**直接复制目录**：把发件包里的 `workspace-example` 整个复制到 `%USERPROFILE%\.pinchbot\workspace\`，即得到与仓库一致的工作区结构（USER.md、AGENTS.md、skills 等）。  
2）**用命令行初始化**：在发件包目录打开终端执行 `PinchBot onboard`，会生成 config 并把**内嵌**的工作区模板写入 `%USERPROFILE%\.pinchbot\workspace\`（无需手动复制）。
...
产物为可执行文件 `PinchBot`、`PinchBot-launcher`、`launcher-chat`（无 .exe），以及 `config/`、`workspace-example/`、`README.txt`。用户数据路径为 `~/.pinchbot/`。
...
1. **PinchBot**
   - 在 `PinchBot/` 下执行 `go generate ./...`（若需嵌入 workspace 等）。
   - 使用 `GOOS=windows GOARCH=amd64` 编译：
     - `PinchBot.exe`（`cmd/picoclaw`）
     - `PinchBot-launcher.exe`（`cmd/picoclaw-launcher`）
2. **Launcher 聊天小窗**
...
注意：GoReleaser 当前只构建 **PinchBot** 内的三个二进制（PinchBot、PinchBot-launcher、PinchBot-launcher-tui），**不包含** Launcher 的 `launcher-chat.exe`。若需要「一个包内同时包含 gateway + 配置页 + 聊天小窗」，请以 **`scripts/build-release.ps1`** 为准，或自行在 CI 中先跑该脚本，再将生成的 `dist/` 目录或 ZIP 作为发布产物。
