    ...
    launcher-chat.app/
      Contents/
        ...
        MacOS/
          ...
          pinchbot-launcher  # 独立设置页后备入口（仅用于调试/兼容）
...
- 数据目录：**程序同目录** 的 **`.openclaw/`**（与 OpenClaw 生态目录名一致）
- 配置：`<数据目录>/config.json`（首次运行自动创建，默认 `workspace: "workspace"`）
- 认证：`<数据目录>/auth.json`
- 工作区：`<数据目录>/workspace/`
...
首次运行自动创建数据目录（默认 `.openclaw`）、默认 `config.json` 与工作区目录；PinchBot 在 Gateway 首次启动时会补齐 starter files。需要覆盖默认位置时，可使用 `PINCHBOT_HOME` / `PINCHBOT_CONFIG`。
...
指定自动更新清单地址（Gitee raw 示例）：

```powershell
.\scripts\build-release.ps1 -Version "1.0.0" -Zip -Installer -UpdateManifestURL "https://gitee.com/rainboxup/pinchbot/raw/main/update/update-manifest.json"
```\n安装器默认安装到 **`%LOCALAPPDATA%\Programs\PinchBot`**，并保留目录选择页，允许你改到其他**用户可写目录**。这样既能提供一个合理默认值，也能在需要时避开路径限制；仍建议不要安装到 `Program Files` 之类受限目录，以免影响程序同目录下的 `.openclaw/` 数据读写。
...
macOS 发布目录现在会包含 `launcher-chat.app`，其内部 `Contents/MacOS/` 会同时放置 `launcher-chat`、`pinchbot`、`pinchbot-launcher`、`platform-server`，从而更接近可分发的 Finder 应用包结构；根目录保留 `config/` 与 `README.txt`。用户数据路径为程序同目录下的 `.openclaw/`。其中 `platform.example.env` 和 `runtime-config.example.json` 仅作为示例，不会自动生效；只有在你显式创建 `config/platform.env` 后，桌面入口才应自动拉起 `platform-server` 并支持登录后的聊天、官方模型、钱包、充值功能。点击“设置”时按需启动的是 `launcher-chat` 内嵌设置页服务，`pinchbot-launcher` 仅作为独立调试/兼容入口，不会在首跑时常驻。若未设置 `MAC_CODESIGN_IDENTITY`，该 `.app` 默认是未签名包，客户侧可能被 Gatekeeper 拦截；正式外发前还应执行 Apple notarization。脚本会把 `git describe` 风格的版本号清洗成 Apple 可接受的 bundle version 字符串，再写入 `Info.plist`，同时把同一版本注入 `launcher-chat` 可见版本号与更新检查逻辑。
...
6. **可选：生成 Windows 安装器**
   ...
   - 默认安装路径为 `%LOCALAPPDATA%\Programs\PinchBot`，但保留目录选择页
