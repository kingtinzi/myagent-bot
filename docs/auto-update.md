...
本文描述 PinchBot 桌面端（launcher-chat + PinchBot 套件）的**自动更新**技术方案：用户打开产品时在后台检查并下载新版本，**下次启动**时使用最新版本，无需手动下载安装包。
...
┌─────────────────────────────────────────────────────────────────┐
│  发布方（你）                                                      │
│  • 构建：scripts/build-release.ps1 → dist/PinchBot-<版本>-.../   │
...
│  • url: 完整 ZIP 下载地址                                          │
│  • zip_folder: ZIP 内顶层目录名（如 "PinchBot-1.0.1-Windows-x86_64"）│
...
│  3. 下载到 %LOCALAPPDATA%\PinchBot\pending\xxx.zip                 │
...
1. **启动时**
   ...
   - 若不需更新：仅记录日志，不做后续动作。
   - 若需要更新：在后台下载 `url` 到 `%LOCALAPPDATA%\PinchBot\pending\<文件名>.zip`，下载完成后写入「待应用」标记（如记录 zip 路径、zip_folder、安装目录）。
...
- **发布流程**：你照常跑脚本得到 `dist/PinchBot-<版本>-Windows-x86_64`，打成 ZIP 上传；同时更新「更新清单」JSON 中的 `version`、`url`、`zip_folder` 即可。
